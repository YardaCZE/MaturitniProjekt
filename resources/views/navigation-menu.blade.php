<nav x-data="{ open: false }" class="bg-[#0D3453] border-b border-[#47663B] shadow-md">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard')" class="text-[#47663B] hover:text-[#EED3B1]">
                {{ __('Dashboard') }}
        ...
        <div class="pt-4 pb-1 border-t border-[#47663B]">
            <div class="flex items-center px-4">
                ...
                <div>
                    <div class="font-medium text-base text-[#47663B]">{{ Auth::user()->name }}</div>
                    <div class="font-medium text-sm text-[#47663B]">{{ Auth::user()->email }}</div>
                </div>
            </div>

            <div class="mt-3 space-y-1">
                <x-responsive-nav-link href="{{ route('profile.show') }}" :active="request()->routeIs('profile.show')" class="text-[#47663B] hover:text-[#EED3B1]">
                    {{ __('Profile') }}
                </x-responsive-nav-link>
                @if (Laravel\Jetstream\Jetstream::hasApiFeatures())
                    <x-responsive-nav-link href="{{ route('api-tokens.index') }}" :active="request()->routeIs('api-tokens.index')" class="text-[#47663B] hover:text-[#EED3B1]">
                        {{ __('API Tokens') }}
                    </x-responsive-nav-link>
                @endif
                <form method="POST" action="{{ route('logout') }}" x-data>
                    @csrf
                    <x-responsive-nav-link href="{{ route('logout') }}" @click.prevent="$root.submit();" class="text-[#47663B] hover:text-[#EED3B1]">
                        {{ __('Log Out') }}
                    </x-responsive-nav-link>
                </form>
            </div>
